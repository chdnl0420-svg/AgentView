...
export function tailAgentOutput(
  ...
  // Backoff for reconnects to avoid burning RAM/CPU when a session is gone
  // for good (worker exited, daemon died). Each miss pushes the next retry
  // out, capped at 30s. The handle stays open so a returning daemon picks
  // up where we left off — we just stop hammering.
  const nextDelay = (): number => Math.min(30_000, 1500 + consecutiveMisses * 1500);
  ...
  const open = async () => {
    ...
    if (!worker) {
      consecutiveMisses++;
      reconnectTimer = setTimeout(open, nextDelay());
      return;
    }
    consecutiveMisses = 0;
    try {
