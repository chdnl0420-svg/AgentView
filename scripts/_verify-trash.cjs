...
(async () => {
  ...
  function call(m,p={}){const i=++id;return new Promise((res,rej)=>{pending.set(i,{res,rej});ws.send(JSON.stringify({id:i,method:m,params:p}))})}
  ...
  const r = await evalJs(`(()=>{
    const left = document.querySelector('.section-head-left');
    if (!left) return null;
    const kids = [...left.children].map((c) => ({ tag: c.tagName, text: (c.textContent || '').trim().slice(0, 30), cls: c.className }));
    return kids;
  })()`);
